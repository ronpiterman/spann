...
public class VisitSuperCallback implements VisitCallback<ClassMetadata> {

	public void perform(Handler<AnnotatedElementMetadata> visitor, ClassMetadata metadata) {
		ClassMetadata superClass = metadata.getSuperClass( false );
		if ( superClass != null ) {
			visitor.handle( superClass );
		}
		
		for ( ClassMetadata ifc : metadata.getInterfaces( false ) ) {
			visitor.handle( ifc );
		}
	}
